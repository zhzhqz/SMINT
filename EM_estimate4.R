...
#xu: the parameter for group identification
...
EM_estimate<-function(TT,YY,Group_NUM,PI_int,Beta,gamma_int,Lamda,Alpha,HYY,lambda,xu)
{
  ...
  while((SStep<60)&(Epsilon>10^(-3))){
    ...
    if(Group_NUM==true_num){
      WZ=order(c(splinex1(xu)%*%Beta[1:q_n,1],splinex1(xu)%*%Beta[1:q_n,2]))
      Beta=Beta[,WZ]
...
if(is.nan(sum(delta))) {  Updated_Est<-list(Group_number=G_1,PI=PI_int,beta=Beta_1,sigma=gamma_1,lamda=Lamda_1,eigencoefficient=Alpha_1,Sstep=100,del=delta_1)
 ...
 ############identification for eigenfunctions
    ...
    Epsilon<-max(max(abs(Beta-Beta_1)),max(abs(gamma_int-gamma_1)),max(abs(PII-PI_1)),max(abs(Alpha-Alpha_1)))
