############################the first derivative for the cumulative distribution of H(y) with respect to H(y)
...
#return: the first derivative for the cumulative distribution of H(y) with respect to H(y)

first_der<-function(hy,PI,alphabeta,sigma,Lamda,Alpha,Group_NUM){
  out=0
  for(i in 1:n){
    XXX<-splinex1(TT[[i]])
    ...
    for(j in 1:length(TT[[i]])){
     ...
     sigmak<-sqrt(Phi[j,]%*%Lamda[,,k]%*%t(t(Phi[j,]))+sigma[k])
        out=out+PI[k]*dnorm((hy-XXX[j,]%*%alphabeta[,k])/sigmak)/sigmak
      }
    }
  }
  return(out)
